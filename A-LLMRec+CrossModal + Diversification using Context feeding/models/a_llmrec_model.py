...
class A_llmrec_model(nn.Module):
    ...
    def cross_modal_attention(self, proj_emb, text_emb):
        """
        Implement the cross-modal attention mechanism where:
        - proj_emb (E_i) is the query (collaborative embedding).
        - text_emb (Q_i) is both key and value (textual embedding).
        
        Formula:
        E_i^{out} = softmax((E_i Q_i^T) / sqrt(d)) Q_i
        """
        # Step 1: Compute the attention scores (Q * K^T)
        attention_scores = torch.matmul(proj_emb, text_emb.T)  # (N, M)

        # Step 2: Scale by sqrt(d) (dimension of embeddings)
        d = proj_emb.size(-1)  # Dimension of the embeddings
        attention_scores = attention_scores / torch.sqrt(torch.tensor(d, dtype=torch.float32, device=proj_emb.device))

        # Step 3: Apply softmax to get attention weights
        attention_weights = F.softmax(attention_scores, dim=-1)

        # Step 4: Compute attended value (attention weights * V)
        attended_text_emb = torch.matmul(attention_weights, text_emb)  # (N, D)

        return attended_text_emb

    def forward(self, proj_emb, text_emb):
        # Example forward pass (use the attention mechanism)
        attended_emb = self.cross_modal_attention(proj_emb, text_emb)
        return attended_emb
